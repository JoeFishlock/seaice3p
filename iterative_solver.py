...
def solve_non_linear_system(
    ...
    iterated_phase_masks = get_phase_masks(
        iterated_enthalpy,
        iterated_salt,
        iterated_gas,
        params,
    ...
    (
        iterated_temperature,
        iterated_liquid_fraction,
        iterated_gas_fraction,
        _,
        iterated_liquid_salinity,
        iterated_dissolved_gas,
    ) = calculate_enthalpy_method(
        iterated_enthalpy, iterated_salt, iterated_gas, params, iterated_phase_masks
    )
    new_permeability = calculate_absolute_permeability(
        geometric(iterated_liquid_fraction)
    )
    pressure_forcing = np.zeros((I + 2,))
    pressure_forcing[1:-1] = (1 / timestep) * (
        iterated_gas_fraction[1:-1] - gas_fraction[1:-1]
    ) + np.matmul(D_e, upwind(iterated_gas_fraction, V))
    pressure_forcing[0] = 0
    pressure_forcing[-1] = 0
    pressure_matrix = np.zeros((I + 2, I + 2))
    perm_matrix = np.zeros((I + 1, I + 1))
